...
//        HttpPost tokenRequest = new HttpPost(applicationUrl + "oauth/token");//где-то задана переменная applicationUrl
        //далее прикрепляется "oauth/token" это значит что мы хотим авториз. на этом сервисе
        //tokenRequest.addHeader("Authorization", "Basic d2ViYXBwOg==");// здесь в addHeader добавляем
        //"Authorization" и "Basic d2ViYXBwOg=="
//        List<NameValuePair> authParams = new ArrayList<>();//эта коллекция будет телом запроса
